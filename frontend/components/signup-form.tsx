    ...
    <form action={formAction} className="rounded-[2rem] border border-[color:var(--border)] bg-[color:var(--surface)] p-6 shadow-[var(--shadow-card)] sm:p-8">
      ...
      <div>
        ...
        <h2 className="mt-3 font-display text-4xl text-[color:var(--foreground)]">
          Join GMT Homes with the role that fits you.
        </h2>
        <p className="mt-3 text-sm leading-7 text-[color:var(--muted)]">
          Buyers and renters can schedule inspections, agents can publish
          listings, and every account keeps its own dashboard history.
        </p>
      </div>
      ...
      <div className="mt-8 space-y-5">
        <label className="block space-y-2">
          ...
          {state.errors?.role ? (
            <p className="text-sm text-[color:#b42318]">{state.errors.role[0]}</p>
          ) : null}
        </label>
