...
import { requestInspectionBookingAction } from "@/app/actions/properties";
...
export function PropertyDetailView({
  ...
  const mapBounds = [
    ...
    property.coordinates.lat - 0.02,
    property.coordinates.lng + 0.02,
    property.coordinates.lat + 0.02,
  ];
  const embeddedMapUrl = `https://www.openstreetmap.org/export/embed.html?bbox=${mapBounds[0]}%2C${mapBounds[1]}%2C${mapBounds[2]}%2C${mapBounds[3]}&layer=mapnik&marker=${property.coordinates.lat}%2C${property.coordinates.lng}`;
  const [bookingState, bookingAction] = useActionState(
    requestInspectionBookingAction,
    initialBookingState,
  );
  const canBookInspection =
    viewer !== null &&
    property.ownerId !== viewer.id &&
    viewer.role !== "admin" &&
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-4 py-10 sm:px-6 lg:px-8">
      ...
      {property.source === "community" && property.moderationStatus !== "approved" ? (
        <div className="rounded-[1.5rem] border border-[color:color-mix(in_oklab,var(--accent)_24%,transparent)] bg-[color:color-mix(in_oklab,var(--accent)_10%,transparent)] px-5 py-4 text-sm text-[color:var(--foreground)]">
          <p className="font-semibold">
            Listing status: {getModerationLabel(property.moderationStatus)}
          </p>
          <p className="mt-2 text-[color:var(--muted)]">
            {property.moderationNotes ??
              "This community listing is still moving through the moderation flow."}
          </p>
        </div>
      ) : null}
      ...
      <section className="grid gap-10 lg:grid-cols-[1.35fr_0.85fr]">
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              {property.source === "community" ? (
                <span className="rounded-full border border-[color:var(--border)] bg-[color:var(--surface)] px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[color:var(--muted)]">
                  {getModerationLabel(property.moderationStatus)}
                </span>
              ) : null}
            </div>
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-[2rem] border border-[color:var(--border)] bg-[color:var(--surface)] p-6 shadow-[var(--shadow-card)] sm:p-8">
            ...
            {viewer ? (
              canBookInspection ? (
                <form action={bookingAction} className="mt-5 space-y-4">
                  ...
                  <label className="block space-y-2">
                    ...
                    />
                    {bookingState.errors?.preferredDate ? (
                      <p className="text-sm text-[color:#b42318]">
                        {bookingState.errors.preferredDate[0]}
                      </p>
                    ) : null}
                  </label>
                  <label className="block space-y-2">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-[color:var(--border)] bg-[color:var(--background)] px-4 py-3 text-sm text-[color:var(--foreground)] outline-none transition focus:border-[color:var(--accent)]"
                    />
                    {bookingState.errors?.preferredTime ? (
                      <p className="text-sm text-[color:#b42318]">
                        {bookingState.errors.preferredTime[0]}
                      </p>
                    ) : null}
                  </label>
                  <label className="block space-y-2">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-[color:var(--border)] bg-[color:var(--background)] px-4 py-3 text-sm text-[color:var(--foreground)] outline-none transition focus:border-[color:var(--accent)]"
                    />
                    {bookingState.errors?.requesterPhone ? (
                      <p className="text-sm text-[color:#b42318]">
                        {bookingState.errors.requesterPhone[0]}
                      </p>
                    ) : null}
                  </label>
                  <label className="block space-y-2">
                    ...
                    <textarea
                      name="message"
                      rows={4}
                      placeholder="Share the times that work for you, what you want to inspect, and anything GMT Homes should prepare."
                    ...
                    />
                    {bookingState.errors?.message ? (
                      <p className="text-sm text-[color:#b42318]">
                        {bookingState.errors.message[0]}
                      </p>
                    ) : null}
                  </label>
                  {bookingState.message ? (
                    <div className="rounded-[1.5rem] border border-[color:color-mix(in_oklab,#b42318_24%,transparent)] bg-[color:color-mix(in_oklab,#b42318_10%,transparent)] px-4 py-4 text-sm text-[color:#b42318]">
                      {bookingState.message}
                    </div>
                  ) : null}
                  {bookingState.successMessage ? (
                    <div className="rounded-[1.5rem] border border-[color:color-mix(in_oklab,var(--accent)_24%,transparent)] bg-[color:color-mix(in_oklab,var(--accent)_10%,transparent)] px-4 py-4 text-sm text-[color:var(--foreground)]">
                      {bookingState.successMessage}
                    </div>
                  ) : null}
                  <FormSubmitButton
                    pendingLabel="Sending request..."
                    className="inline-flex w-full items-center justify-center rounded-full bg-[color:var(--accent-strong)] px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    Request inspection
                  </FormSubmitButton>
                </form>
              ) : (
                <div className="mt-5 rounded-[1.5rem] bg-[color:var(--surface-strong)] px-4 py-4 text-sm leading-7 text-[color:var(--foreground)]">
                  {property.ownerId === viewer.id
                    ? "This is your listing, so inspection requests will appear in your dashboard instead of this form."
                    : "Admin accounts can review incoming inspection requests from the dashboard and moderation screens."}
                </div>
              )
            ) : (
              <div className="mt-5 space-y-4">
                <p className="text-sm leading-7 text-[color:var(--muted)]">
                  Sign in as a buyer, renter, or agent to request an inspection
                  without leaving the property page.
                </p>
                <Link
                  href={`/login?next=${encodeURIComponent(`/properties/${property.slug}`)}`}
                  className="inline-flex w-full items-center justify-center rounded-full bg-[color:var(--accent-strong)] px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5"
                >
                  Sign in to book
                </Link>
              </div>
          ...
          <section className="rounded-[2rem] border border-[color:var(--border)] bg-[color:var(--surface)] p-6 shadow-[var(--shadow-card)] sm:p-8">
            ...
            <p className="mt-3 text-sm leading-7 text-[color:var(--muted)]">
              Explore the surrounding area directly from the listing with an
              embedded map instead of leaving the property page just to orient
              yourself.
            </p>
